...
function hideBtnCancel() {
  ...
  if(document.querySelector('.btn-show')){
    document.querySelector('.btn-show').style.display = 'inline-block';
  }
    ...
    $.getJSON($("#ListaUsuariosURL").val(), pdata, function (data) {
        if (data.Result.length > 0) {
          $.each(data.Result, function (index, dados) {
            ...
            tabela = `<tr><td style="width:30px; cursor: pointer;"><a onclick="EditarUsuario('${(dados.usr_Id !== null) ? dados.usr_Id : " "}','${(dados.usr_Login !== null) ? dados.usr_Login : " "}','${(dados.usr_Nome !== null) ? dados.usr_Nome : " "}','${(dados.usr_Email !== null) ? dados.usr_Email : " "}','${(dados.usr_Tel !== null) ? dados.usr_Tel : " " }','${(dados.fnc_Nome !== null) ? dados.fnc_Nome : " "}','${(dados.estr_Nome !== null) ? dados.estr_Nome : " "}','${str}','${strs}','${dados.usr_imagePerfil}')"><img src='../images/edit.svg'></a>`;
            tabela = tabela + "</td><td style=\"width:30px;cursor: pointer;\"><a onclick=\"ResetSenhaOutroUser('" + dados.usr_Id + "')\"><img src='../images/lock.svg'></a>";
...
function SalvaUsuario() {
    ...
    if(pdata["acessos"] == 1) {
      pdata["acessos"] = [1,5,6,9,10,11,12,13,14,15,16].toString();
      pdata.perfil = 'gestor';
...
function OrganizeTable(tb, index = 0, controllers) {
  if(document.querySelector(`.${tb}`)){
    ...
    con.innerHTML = "";
    for(let i = 0; i < qtdPgs; ++i){
        con.innerHTML += `<button onclick="OrganizeTable(this.parentNode.parentNode.classList[0], this.innerHTML, 'controllers')" class="btn btn-secondary">${i+1}</button>`;
    }
  }
}
