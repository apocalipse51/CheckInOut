angular.module('checkin').controller('ProjetoController', function($scope, $http, $window) {
    ...
    $http.get(`/horasequipe/projetos?DtInicio=${i}&DtFim=${f}`).then(function(res) {
        $scope.options = $scope.Unique(res.data.Result);
        createOptions($scope.options, 'projetos');
        $scope.makeSearch('Projeto', $scope.options[0]);
    });
    ...
    $scope.makeSearch = function(field, value) {
        ...
        let valueToFind = value.toLowerCase().replace(" ", "");
        let service = `/HorasEquipe/projetosfind?DtInicio=${inicial}&DtFim=${final}&projeto=${valueToFind}`;                
    ...
    $scope.search = function() {
        let value = document.querySelector('.projetos').value;
